...
/// A monadic type wrapping a [syn::File] abstract syntax tree (AST)
/// whose monadic functions are passes that mutate the monad's held
/// AST value.
///
/// (See https://en.wikipedia.org/wiki/Monad_(functional_programming)
/// for more background on monads.)
pub struct MonadicAst {
    pub ast: File
}

impl MonadicAst {
    /// Returns a monadic value wrapping an AST constructed from the given Rust code.
    ...
    pub fn convert_ffi_types(mut self) -> Self {
...
impl From<File> for MonadicAst {
    ...
    ///
    /// Receives a `syn::File` AST and wraps it into a monadic value, where
    /// `M` is a `MonadicAst` and `a` is a `syn::File` abstract syntax trees.
...
/// (or `>>=`) operation.
///
/// Receives a monadic AST wrapper `M a` and returns the result `M b` of
/// applying the `bind()` method on the unwrapped AST `a`, where `M` is a
/// `MonadicAst` and `a`,`b` are `syn::File` abstract syntax trees.
pub trait Pass {
